...
public class AghEmailValidator implements Validator {

    private static final String AGH_EMAIL_PATTERN =
            "^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@"
                    + "([A-Za-z0-9-]+\\.)?(agh\\.edu\\.pl)$";

    @Override
    public ValidationResult validate(Field field) {
        EditText textView = field.getTextView();
        boolean isValid = textView.getText().toString().matches(AGH_EMAIL_PATTERN);
        return isValid ?
                ValidationResult.success(textView)
                : ValidationResult.failure(textView, field.getContext().getString(R.string.validator_not_agh_email));
    }
}